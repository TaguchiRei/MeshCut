...
namespace UsefulMeshCut
{
    ...
    public class MultiCutBlade : MonoBehaviour
    {
        ...
        private async void Test()
        {
            ...
            {
                Debug.Log("[UsefulMeshCut] 範囲内にCuttableObjectが見つかりませんでした");
            }
        }
        ...
        public async UniTask ExecuteCut(CuttableObject[] targets)
        {
            ...
            {
                Debug.LogError(
                    $"[UsefulMeshCut] 破片が不足しています。必要数 {requiredCount} に対し取得数 {fragmentStubs.Count}。プールの生成数を増やしてください。");
                return;
            }
            ...
            {
                Debug.Log($"[UsefulMeshCut] 切断から反映までの全体処理時間 {st.ElapsedMilliseconds}ms");
            }
        }
        ...
        private static CuttableObject[] FilterCuttable(CuttableObject[] targets)
        {
            ...
            foreach (CuttableObject target in targets)
            {
                ...
                {
                    Debug.LogWarning($"[UsefulMeshCut] {target.name} は既に切断済みのため除外しました。");
                    continue;
                }
        ...
        private async UniTask CheckTime(Stopwatch stopwatch, float limitMs = 5f)
        {
            if (stopwatch.ElapsedMilliseconds > limitMs)
            {
                ...
                if (_enableProfileLog)
                {
                    Debug.Log("[UsefulMeshCut] 処理時間が長すぎたため、次のフレームに送りました。");
                }
            }
        }
