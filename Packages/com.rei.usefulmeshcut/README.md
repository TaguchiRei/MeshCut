...
Burst / Collections / Mathematics は本パッケージの依存として自動的に解決されます。

## セットアップ

メニューから `UsefulTools > UsefulMesh > MeshCut > Setup` を開きます。

1. **シーンの準備** — 破片プレハブとプール生成数を指定して「シーンをセットアップ」を押すと、
   `MeshDataCache` / `FragmentPool` / `CutBlade` が生成され、相互参照が設定されます。
