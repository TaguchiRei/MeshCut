...
namespace UsefulMeshCut.Editor
{
    /// <summary>
    /// UsefulMeshCut を使うためのシーンセットアップと、選択オブジェクトの切断可能化を行うウィンドウ。
    /// </summary>
    public class MeshCutSetupWindow : EditorWindow
    {
        private const string SystemRootName = "UsefulMeshCut System";
        ...
        [MenuItem("UsefulTools/UsefulMesh/MeshCut/Setup")]
        ...
        private void SetupScene()
        {
            ...
            Debug.Log("[UsefulMeshCut] シーンのセットアップが完了しました。");
        }
        ...
        private void AddCuttableToSelected()
        {
            ...
            {
                Debug.LogError("[UsefulMeshCut] シーンに MeshDataCache がありません。先に「シーンをセットアップ」を実行してください。");
                return;
            }
            ...
            foreach (GameObject obj in Selection.gameObjects)
            {
                ...
                {
                    Debug.LogWarning($"[UsefulMeshCut] {obj.name} はプレハブアセットのためスキップしました。");
                    continue;
                }
                ...
                {
                    Debug.LogWarning($"[UsefulMeshCut] {obj.name} は MeshFilter または Renderer が無いためスキップしました。");
                    continue;
                }

                if (meshFilter.sharedMesh == null)
                {
                    Debug.LogWarning($"[UsefulMeshCut] {obj.name} は Mesh が未設定のためスキップしました。");
                    continue;
                }
            ...
            Debug.Log($"[UsefulMeshCut] {count} 個のオブジェクトを切断可能化しました。");
        }
        ...
        private void RemoveCuttableFromSelected()
        {
            ...
            Debug.Log($"[UsefulMeshCut] {count} 個のオブジェクトから CuttableObject を削除しました。");
        }
