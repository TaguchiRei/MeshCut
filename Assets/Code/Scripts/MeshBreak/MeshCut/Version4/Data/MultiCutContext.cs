...
{
    /// <summary>
    /// MultiMeshCut.Cut 1回の呼び出しで使う全NativeバッファをまとめたVersion4版コンテキスト。
    /// V2のMultiCutContextと同じ役割だが、全処理をJob化するため中間データも全てNative化されている。
    /// </summary>
    public class MultiCutContext : IDisposable
    {
        ...
        public void AllocateFragmentBuffers(int maxSubmeshSlots)
        {
            ...
            for (int objIndex = 0; objIndex < ObjectCount; objIndex++)
            {
                ...
                // 安全な上限: 元三角形(≤3*triCount) + 断面新規三角形(≤9*triCount)。キャップスロットにも同じ上限を流用する。
                ...
                for (int side = 0; side < 2; side++)
                {
                    ...
                    for (int s = 0; s < maxSubmeshSlots; s++)
                    {
                        ...
                        FragmentIndexRange[slot] = new int2(idxTotal, idxCap);
                        idxTotal += idxCap;
                    }
                }
            }
