...
namespace MeshBreak.MeshCut.Version4
{
    ...
    /// V2の「新三角形振り分け(plainループ)」+「FindAllLoops」+「FillCapFan」を1つのJobへ統合したもの。
    ...
    public struct DistributeAndCapJob : IJobParallelFor
    {
        ...
        public void Execute(int objIndex)
        {
            ...
            int capSubmesh = ObjectSubmeshCount[objIndex];
        ...
        /// <summary> dedupなしで3頂点を追加する(V2の BurstBreakMesh.AddTriangle と同じ)。更新後の頂点カーソルを返す。 </summary>
